...
import { Flame, CheckCircle2, ShieldCheck, BarChart3, Settings, Info, Pause } from 'lucide-react';
  ...
  return (
    <div className="space-y-8 fade-in pb-20">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        <div className="glass-surface p-8 rounded-[2.5rem] relative overflow-hidden group border border-white/10">
          <Flame className={`absolute bottom-[-10px] right-[-10px] w-28 h-28 opacity-10 transition-transform duration-700 group-hover:scale-125 ${isEthereal ? 'text-[#10b981]' : 'text-[#00E5FF]'}`} />
        ...
        <div className="glass-surface p-8 rounded-[2.5rem] border border-white/10">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 glass-surface rounded-[2.5rem] overflow-hidden flex flex-col border border-white/10">
          <div className={`px-8 py-6 border-b flex items-center justify-between ${isEthereal ? 'border-slate-100 bg-white/30' : 'border-white/5 bg-white/5'}`}>
            ...
            <button className={`text-[10px] font-black uppercase tracking-widest flex items-center transition-all ${isEthereal ? 'text-[#10b981] hover:text-[#059669]' : 'text-[#00E5FF] hover:text-cyan-300'}`}>
              ...
              <tbody className={`divide-y ${isEthereal ? 'divide-slate-100' : 'divide-white/5'}`}>
                {[
                  ...
                  <tr key={i} className={`${isEthereal ? 'hover:bg-[#10b981]/5' : 'hover:bg-white/5'} transition-colors group`}>
                    <td className={`px-8 py-6 font-bold text-sm ${isEthereal ? 'text-[#064e3b]' : 'text-slate-200'}`}>{node.email}</td>
                    <td className="px-8 py-6">
                      <div className="flex items-center space-x-3">
                        <div className={`w-32 h-1.5 rounded-full overflow-hidden ${isEthereal ? 'bg-slate-200' : 'bg-white/10'}`}>
                           <div className={`h-full ${primaryColor === '#10b981' ? 'bg-[#10b981]' : 'bg-[#00E5FF]'}`} style={{ width: '60%' }}></div>
                        </div>
        ...
        <div className="glass-surface p-8 rounded-[2.5rem] flex flex-col border border-white/10">
           ...
           <div className="flex-1 min-h-[220px]">
              <ResponsiveContainer width="100%" height="100%">
                 <AreaChart data={rampData}>
                    ...
                    <Tooltip 
                      contentStyle={{ backgroundColor: isEthereal ? '#fff' : '#1e293b', border: 'none', borderRadius: '16px', boxShadow: '0 10px 15px -3px rgba(0,0,0,0.1)' }} 
           ...
           <div className={`mt-8 p-6 rounded-[1.5rem] border ${isEthereal ? 'bg-[#10b981]/5 border-[#10b981]/10' : 'bg-white/5 border-white/10'}`}>
              <p className={`text-[10px] font-bold leading-relaxed ${isEthereal ? 'text-slate-600' : 'text-slate-400'}`}>
                 <Info size={14} className={`inline mr-2 ${isEthereal ? 'text-emerald-500' : 'text-cyan-500'}`} /> AI Nodes simulate realistic thread depth (1-4 replies) to maximize authority scores across ISP networks.
              </p>
           </div>
        </div>
      </div>
