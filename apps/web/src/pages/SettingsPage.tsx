...
import { User, Building, CreditCard, Shield, Key, Users, Globe, Crown, Check } from 'lucide-react';
        ...
        <div className="w-full lg:w-72 space-y-2">
          {menuItems.map((item) => (
            <button
              ...
              className={`w-full flex items-center px-6 py-4 rounded-2xl transition-all font-black text-sm ${activeMenu === item.name
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  {plans.map((plan) => (
                    <div key={plan.name} className={`glass-surface p-8 rounded-[2.5rem] border flex flex-col ${plan.current ? 'border-[#10b981]' : 'border-white/5 opacity-80'}`}>
                      ...
                      <button className={`w-full py-3 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${plan.current
