...
const LoginPage: React.FC<{ onLogin: () => void }> = ({ onLogin }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Simulate API logic
    setTimeout(() => {
      localStorage.setItem('token', 'stub-jwt-token');
  ...
  return (
    <div className="min-h-screen theme-ethereal flex items-center justify-center p-6">
      ...
      >
        ...
        {error && (
          <div className="mb-8 p-4 bg-rose-500/10 border border-rose-500/20 text-rose-600 text-xs font-black uppercase tracking-widest rounded-2xl flex items-center">
        ...
        <div className="mt-12 flex items-center justify-center space-x-3 text-[10px] text-slate-400 font-black uppercase tracking-[0.2em]">
           <ShieldCheck size={14} className="text-[#10b981]" />
           <span>Level 4 Encryption Active</span>
        </div>
      </motion.div>
    </div>
  );
