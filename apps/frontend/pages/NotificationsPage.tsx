  ...
  return (
    <div className="space-y-8 fade-in pb-20 max-w-5xl mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
        <div>
          <h1 className={`text-3xl md:text-4xl font-black font-heading tracking-tight ${isEthereal ? 'text-[#064e3b]' : 'text-white'}`}>Alert Center</h1>
          <p className={`${isEthereal ? 'text-slate-600' : 'text-slate-400'} font-medium`}>Workspace integrity alerts and deliverability notifications.</p>
        </div>
        <button className={`px-6 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest border transition-all ${
          isEthereal ? 'bg-white border-slate-100 text-slate-700' : 'bg-white/5 border-white/10 text-white'
        }`}>
      ...
      <div className="space-y-4">
        <AnimatePresence>
          {notifications.map((notif) => (
            <motion.div
              ...
              className={`glass-surface p-6 rounded-[2rem] border-l-[6px] flex items-start gap-6 transition-all ${
                notif.type === 'critical' ? 'border-l-rose-500' : 'border-l-amber-500'
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between mb-2">
                  <h3 className={`text-base font-black ${isEthereal ? 'text-[#064e3b]' : 'text-white'}`}>{notif.title}</h3>
                  <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">{notif.time}</span>
                </div>
                <p className={`text-sm font-medium leading-relaxed ${isEthereal ? 'text-slate-600' : 'text-slate-400'}`}>
                  {notif.message}
                </p>
                <div className="mt-6 flex items-center space-x-6">
                   <button className={`text-[10px] font-black uppercase tracking-[0.2em] transition-all flex items-center ${isEthereal ? 'text-[#10b981]' : 'text-[#00E5FF]'}`}>
              ...
              <button 
                ...
                className={`p-3 rounded-2xl transition-all ${isEthereal ? 'hover:bg-slate-100 text-slate-400' : 'hover:bg-white/10 text-slate-500'}`}
              >
                <Trash2 size={18} />
              </button>
            </motion.div>
        ...
        {notifications.length === 0 && (
          <div className="py-24 text-center">
             <div className={`w-20 h-20 rounded-[2rem] flex items-center justify-center mx-auto mb-6 ${isEthereal ? 'bg-emerald-50 text-emerald-200' : 'bg-white/5 text-slate-700'}`}>
                <CheckCircle2 size={40} />
             </div>
             <h3 className={`text-2xl font-black font-heading ${isEthereal ? 'text-[#064e3b]' : 'text-white'}`}>Integrity Verified</h3>
