...
import { Bell, Search, ChevronDown, UserCircle, Menu, Sparkles, Sun } from 'lucide-react';
...
const Navbar: React.FC<NavbarProps> = ({ workspace, theme, onToggleTheme, onToggleSidebar }) => {
  ...
  return (
    <header className={`h-20 flex items-center justify-between px-8 z-30 sticky top-0 w-full transition-all duration-500`}>
      <div className={`flex-1 flex items-center justify-between px-6 h-16 glass-surface rounded-3xl`}>
        <div className="flex items-center space-x-4">
          <button onClick={onToggleSidebar} className={`p-2 lg:hidden ${isEthereal ? 'text-slate-600' : 'text-slate-300'}`}>
            <Menu size={24} />
          </button>
          <div className="flex items-center space-x-3 px-3 py-1.5 rounded-2xl bg-white/20 border border-white/40 cursor-pointer hover:shadow-sm transition-all">
        ...
        <div className="flex-1 max-w-lg mx-10 relative hidden md:block">
          ...
          <input 
            ...
            className={`w-full h-10 pl-12 pr-4 rounded-2xl text-xs font-medium focus:outline-none transition-all ${
              isEthereal ? 'bg-white/40 border border-white/60 text-slate-700' : 'bg-white/5 border border-white/10 text-white'
            }`}
          />
        </div>
