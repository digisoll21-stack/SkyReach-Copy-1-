...
const Sidebar: React.FC<SidebarProps> = ({ workspace, theme, onClose }) => {
  ...
  const navItems = [
    ...
    { name: 'Inboxes', path: '/inboxes', icon: Mail },
  ...
  return (
    <aside className={`w-72 h-full flex flex-col transition-all duration-500 m-4 rounded-[2.5rem] glass-surface overflow-hidden`}>
      ...
      <nav className="flex-1 px-5 py-4 space-y-1.5 overflow-y-auto">
        {navItems.map((item) => (
          <NavLink
            ...
            onClick={onClose}
            className={({ isActive }) =>
              `flex items-center px-5 py-3.5 rounded-2xl transition-all duration-300 group font-bold text-sm ${
                isActive 
      ...
      <div className={`px-6 py-8 mt-auto ${isEthereal ? 'border-t border-[#10b981]/10 bg-[#10b981]/5' : 'border-t border-white/5 bg-white/5'}`}>
        <div className="mb-6 px-1">
          ...
          <div className={`w-full h-2 rounded-full overflow-hidden ${isEthereal ? 'bg-white' : 'bg-slate-800'}`}>
        ...
        <NavLink
          ...
          onClick={onClose}
          className={({ isActive }) =>
            `flex items-center px-5 py-3.5 rounded-2xl transition-all duration-200 group font-bold text-sm ${
              isActive 
